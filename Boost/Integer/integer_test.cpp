...
const char* get_name_of_type(long_long_type)
{
    return "boost::long_long_type";
}
const char* get_name_of_type(ulong_long_type)
{
    return "boost::ulong_long_type";
}
#endif

template <int Bits>
//void do_test_exact(boost::mpl::true_ const&)
void do_test_exact(mpl::true_ const&)
{
   ...
   typedef typename int_t<Bits>::exact int_exact;
   typedef typename uint_t<Bits>::exact uint_exact;
   typedef typename int_t<Bits>::least least_int;
   typedef typename uint_t<Bits>::least least_uint;
...
void do_test_exact(mpl::false_ const&)
{
...
void do_test_bits()
{
   ...
   do_test_exact<Bits>(
      ...
      (sizeof(ulong_long_type) * CHAR_BIT == Bits)
#endif
      >());
   //
...
//    typedef typename boost::uint_t<Bits>::fast fast_uint;
   typedef typename int_t<Bits>::least least_int;
   typedef typename int_t<Bits>::least fast_int;
   typedef typename uint_t<Bits>::least least_uint;
   typedef typename uint_t<Bits>::fast fast_uint;   

   //if(std::numeric_limits<least_int>::is_specialized)
   if(numeric_limits<least_int>::is_specialized)
   {
      //BOOST_TEST(std::numeric_limits<least_int>::digits + 1 >= Bits);
      BOOST_TEST(numeric_limits<least_int>::digits + 1 >= Bits);
   }
   //if(std::numeric_limits<least_uint>::is_specialized)
   if(numeric_limits<least_uint>::is_specialized)
   {
      //BOOST_TEST(std::numeric_limits<least_uint>::digits >= Bits);
      BOOST_TEST(numeric_limits<least_uint>::digits >= Bits);
   }
   ...
   if(detail::test_errors() != last_error_count)
   {
    ...
    //   std::cout << "Type uint_t<" << Bits << ">::fast was " << get_name_of_type(fast_uint(0)) << std::endl;
      last_error_count = detail::test_errors();
      cout << "Errors occurred while testing with bit count = " << Bits << endl;
      cout << "Type int_t<" << Bits << ">::least was " << get_name_of_type(least_int(0)) << endl;
      cout << "Type int_t<" << Bits << ">::fast was " << get_name_of_type(fast_int(0)) << endl;
      cout << "Type uint_t<" << Bits << ">::least was " << get_name_of_type(least_uint(0)) << endl;
      cout << "Type uint_t<" << Bits << ">::fast was " << get_name_of_type(fast_uint(0)) << endl;    
   }
}
template <>
void do_test_bits<-1>()
...
//    BOOST_TEST((std::numeric_limits<least_type>::max)() >= val);
   BOOST_TEST((numeric_limits<least_type>::min)() <= val);
   BOOST_TEST((numeric_limits<least_type>::max)() >= val);
   //if(boost::detail::test_errors() != last_error_count)
   if(detail::test_errors() != last_error_count)
   {
    //   last_error_count = boost::detail::test_errors();
    ...
    //   std::cout << "Required value is: " << val << std::endl;
      last_error_count = detail::test_errors();
      cout << "Traits type is: " << typeid(Traits).name() << endl;
      cout << "Least type is: " << get_name_of_type(least_type(0)) << endl;
      cout << "Fast type is: " << get_name_of_type(fast_type(0)) << endl;
      cout << "Expected type is: " << get_name_of_type(Expected(0)) << endl;
      cout << "Required value is: " << val << endl;    
   }
}
...
//       do_test_bits<std::numeric_limits<boost::uintmax_t>::digits>();
   if(numeric_limits<intmax_t>::is_specialized)
      do_test_bits<numeric_limits<uintmax_t>::digits>();
   else
      //do_test_bits<std::numeric_limits<long>::digits>();
      do_test_bits<numeric_limits<long>::digits>();  
   //
...
//    test_min_max_type<boost::uint_value_t<UCHAR_MAX>, unsigned char>(UCHAR_MAX);
   test_min_max_type<int_max_value_t<SCHAR_MAX>, signed char>(SCHAR_MAX);
   test_min_max_type<int_min_value_t<SCHAR_MIN>, signed char>(SCHAR_MIN);
   test_min_max_type<uint_value_t<UCHAR_MAX>, unsigned char>(UCHAR_MAX);
    ...
    //   test_min_max_type<boost::uint_value_t<USHRT_MAX>, unsigned short>(USHRT_MAX);
      test_min_max_type<int_max_value_t<SCHAR_MAX+1>, short>(SCHAR_MAX+1);
      test_min_max_type<int_min_value_t<SCHAR_MIN-1>, short>(SCHAR_MIN-1);
      test_min_max_type<uint_value_t<UCHAR_MAX+1>, unsigned short>(UCHAR_MAX+1);
      test_min_max_type<int_max_value_t<SHRT_MAX>, short>(SHRT_MAX);
      test_min_max_type<int_min_value_t<SHRT_MIN>, short>(SHRT_MIN);
      test_min_max_type<uint_value_t<USHRT_MAX>, unsigned short>(USHRT_MAX);    
#endif
    ...
    //   test_min_max_type<boost::uint_value_t<UINT_MAX>, unsigned int>(UINT_MAX);
      test_min_max_type<int_max_value_t<SHRT_MAX+1>, int>(SHRT_MAX+1);
      test_min_max_type<int_min_value_t<SHRT_MIN-1>, int>(SHRT_MIN-1);
      test_min_max_type<uint_value_t<USHRT_MAX+1>, unsigned int>(USHRT_MAX+1);
      test_min_max_type<int_max_value_t<INT_MAX>, int>(INT_MAX);
      test_min_max_type<int_min_value_t<INT_MIN>, int>(INT_MIN);
      test_min_max_type<uint_value_t<UINT_MAX>, unsigned int>(UINT_MAX);    
#endif
    ...
    //   test_min_max_type<boost::uint_value_t<ULONG_MAX>, unsigned long>(ULONG_MAX);
      test_min_max_type<int_max_value_t<INT_MAX+1L>, long>(INT_MAX+1L);
      test_min_max_type<int_min_value_t<INT_MIN-1L>, long>(INT_MIN-1L);
      test_min_max_type<uint_value_t<UINT_MAX+1uL>, unsigned long>(UINT_MAX+1uL);
      test_min_max_type<int_max_value_t<LONG_MAX>, long>(LONG_MAX);
      test_min_max_type<int_min_value_t<LONG_MIN>, long>(LONG_MIN);
      test_min_max_type<uint_value_t<ULONG_MAX>, unsigned long>(ULONG_MAX);    
#endif
    ...
    //   test_min_max_type<boost::uint_value_t<ULLONG_MAX>, boost::ulong_long_type>(ULLONG_MAX);
      test_min_max_type<int_max_value_t<LONG_MAX+1LL>, boost::long_long_type>(LONG_MAX+1LL);
      test_min_max_type<int_min_value_t<LONG_MIN-1LL>, boost::long_long_type>(LONG_MIN-1LL);
      test_min_max_type<uint_value_t<ULONG_MAX+1uLL>, boost::ulong_long_type>(ULONG_MAX+1uLL);
      test_min_max_type<int_max_value_t<LLONG_MAX>, boost::long_long_type>(LLONG_MAX);
      test_min_max_type<int_min_value_t<LLONG_MIN>, boost::long_long_type>(LLONG_MIN);
      test_min_max_type<uint_value_t<ULLONG_MAX>, boost::ulong_long_type>(ULLONG_MAX);    
#endif
    ...
    //   test_min_max_type<boost::uint_value_t<ULONG_LONG_MAX>, boost::ulong_long_type>(ULONG_LONG_MAX);
      ...
      test_min_max_type<int_min_value_t<LONG_MIN-1LL>, boost::long_long_type>(LONG_MIN-1LL);
      test_min_max_type<uint_value_t<ULONG_MAX+1uLL>, boost::ulong_long_type>(ULONG_MAX+1uLL);
      test_min_max_type<int_max_value_t<LONG_LONG_MAX>, boost::long_long_type>(LONG_LONG_MAX);
      test_min_max_type<int_min_value_t<LONG_LONG_MIN>, boost::long_long_type>(LONG_LONG_MIN);
      test_min_max_type<uint_value_t<ULONG_LONG_MAX>, boost::ulong_long_type>(ULONG_LONG_MAX);    
#endif
    ...
    //   test_min_max_type<boost::uint_value_t<ULONGLONG_MAX>, boost::ulong_long_type>(ULONGLONG_MAX);
      ...
      test_min_max_type<int_min_value_t<LONG_MIN-1LL>, boost::long_long_type>(LONG_MIN-1LL);
      test_min_max_type<uint_value_t<ULONG_MAX+1uLL>, boost::ulong_long_type>(ULONG_MAX+1uLL);
      test_min_max_type<int_max_value_t<LONGLONG_MAX>, boost::long_long_type>(LONGLONG_MAX);
      test_min_max_type<int_min_value_t<LONGLONG_MIN>, boost::long_long_type>(LONGLONG_MAX);
      test_min_max_type<uint_value_t<ULONGLONG_MAX>, boost::ulong_long_type>(ULONGLONG_MAX);    
#endif
    ...
    //   test_min_max_type<boost::uint_value_t<_ULLONG_MAX>, boost::ulong_long_type>(_ULLONG_MAX);
      ...
      test_min_max_type<int_min_value_t<LONG_MIN-1LL>, boost::long_long_type>(LONG_MIN-1LL);
      test_min_max_type<uint_value_t<ULONG_MAX+1uLL>, boost::ulong_long_type>(ULONG_MAX+1uLL);
      test_min_max_type<int_max_value_t<_LLONG_MAX>, boost::long_long_type>(_LLONG_MAX);
      test_min_max_type<int_min_value_t<_LLONG_MIN>, boost::long_long_type>(_LLONG_MIN);
      test_min_max_type<uint_value_t<_ULLONG_MAX>, boost::ulong_long_type>(_ULLONG_MAX);    
