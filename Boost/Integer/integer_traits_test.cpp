...
void runtest(const char * type, T)
{
  ...
  typedef integer_traits<T> traits;
...
int main(int, char*[])
{
  ...
  runtest("int64_t (possibly long long)", int64_t());
  runtest("uint64_t (possibly unsigned long long)", uint64_t());
#else
  //std::cout << "Skipped int64_t and uint64_t" << std::endl;
  cout << "Skipped int64_t and uint64_t" << endl;
#endif
